...
def worker(conn, env):
    """
    The worker class interacts with each environment individually and sends the result of the interaction for procedurally-generated envs
    """
    while True:
        cmd, data = conn.recv()
        if cmd == "step":
...
class SingletonParallelEnv(gym.Env):
    """A concurrent execution of environments in multiple processes for singleton envs.
    The ParallelEnv class manages the workers and controls the interactions of multiple workers at the same time
    """
